...
<header class="masthead">
	<div class="container">
		<div class="masthead-subheading">Welcome To Travel Yatri</div>
...
<section class="page-section bg-dark" id="home">
	...
	<div class="row">
		<?php
		$packages = $conn->query("SELECT * FROM `packages` order by rand() limit 5	");
			while($row = $packages->fetch_assoc() ):
				...
				if(is_dir(base_app.'uploads/package_'.$row['id'])){
					...
					if($k !== false)
						unset($img[$k]);
					$cover = isset($img[4]) ? 'uploads/package_'.$row['id'].'/'.$img[4] : "";
				}
